...
class BasicModelLabel(nn.Module):
    def __init__(self, bert_name, cfg, model_cfg, **other_bert_kwargs):
        ...
        self.label_fc1 = nn.Linear(w_mat.shape[1], model_cfg['label_emb_fc_1_out'])

        self.feature_fc1 = nn.Linear(2048, model_cfg['feature_emb_fc_1_out'])

        self.pos_emb_layer = nn.Linear(5, model_cfg['pos_emb_size'])
        self.img_bilstm = nn.LSTM(
            model_cfg['feature_emb_fc_1_out'] + model_cfg['pos_emb_size'] + model_cfg['label_emb_fc_1_out'],
    ...
    def forward(self, query, box_pos, box_feature, box_label):
        ...
        label_emb = label_emb.view(-1, self.label_emb.embedding_dim)
        label_emb = F.leaky_relu(self.label_fc1(label_emb))
        label_emb = label_emb.view(batch_size, -1, self.model_cfg['label_emb_fc_1_out'])

        feature_emb = box_feature.view(-1, 2048)
        feature_emb = F.leaky_relu(self.feature_fc1(feature_emb))
        feature_emb = feature_emb.view(batch_size, -1, self.model_cfg['feature_emb_fc_1_out'])

        image_seq_feat = torch.cat([pos_emb, feature_emb, label_emb], dim=2)
        ...
        embs = F.leaky_relu(self.clf1(embs))
        embs = F.leaky_relu(self.clf2(embs))
